...
export const invite = catchAsync(
  async (req: customRequest, res: Response, next: NextFunction) => {
    ...
    const user = (await User.findOne({ email: invitee.toLowerCase() })) as {
      _id: string;
      username: string;
      email: string;
    };
    ...
    if (groupId) {
      if (!alreadyFriend)
        return next(new AppError("You can only invite your friends!", 409));
...
export const accept = catchAsync(
  async (req: customRequest, res: Response, next: NextFunction) => {
    ...
    if (groupId) {
      ...
      ).lean()) as { _id: string; friends: friends[] };
